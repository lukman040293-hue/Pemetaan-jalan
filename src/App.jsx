...
// (Catatan: Jika tombol pratinjau hilang, refresh halaman web browser Anda)
// =========================================================================
...
export default function App() {
  ...
  const showToast = (message) => {
    // Memastikan message berupa string agar tidak memicu error render React
    setToastMessage(String(message));
    setTimeout(() => setToastMessage(null), 4000);
  };

  // --- INISIALISASI SUPABASE KLIEN (CDN) ---
  useEffect(() => {
  ...
  const fetchRoads = async () => {
    ...
    try {
      ...
      // ✅ PERBAIKAN BUG: Parse data stringifikasi dari database
      // Supabase agar kembali menjadi Array/Objek yang valid untuk diproses Peta
      const formattedData = (data || []).map((road) => {
        ...
        if (typeof parsedGps === 'string') {
          try {
            parsedGps = JSON.parse(parsedGps);
          } catch (e) {
            parsedGps = [];
          }
        }
        if (typeof parsedPin === 'string') {
          try {
            parsedPin = JSON.parse(parsedPin);
          } catch (e) {
            parsedPin = null;
          }
        }

        // Paksa bentuk Array agar .map() di pembuat Polyline Leaflet tidak error
      ...
      console.warn(
        'Peringatan jaringan saat mengambil data dari Supabase:',
        error.message
      );
      ...
      if (
        ...
        showToast(
          "Gagal menyambung. Server Supabase Anda mungkin sedang 'Paused' (Jeda). Silakan aktifkan kembali di dasbor."
        );
  ...
  useEffect(() => {
    if (supabase) {
      ...
      // --- PENGGANTI REALTIME: POLLING ---
      const intervalId = setInterval(() => {
        fetchRoads();
      }, 5000);

      return () => {
        clearInterval(intervalId); // Bersihkan interval saat komponen dibongkar
      };
    }
  }, [supabase]);
  ...
  useEffect(() => {
    if (
      appRole !== 'admin' ||
      !isLeafletLoaded ||
      !adminMapContainerRef.current
    )
      return;

    const map = window.L.map(adminMapContainerRef.current).setView(
      [-0.425, 117.185],
      13
    );
  ...
  useEffect(() => {
    ...
    const filteredRoads = syncedRoads.filter((road) => {
      return (
        (filterKelurahan === 'Semua' || road.kelurahan === filterKelurahan) &&
    ...
    filteredRoads.forEach((road) => {
      if (road.realGps && road.realGps.length > 0) {
        // Karena realGps sekarang dijamin selalu berupa Array (dari parser di atas), error map() akan hilang
        ...
        if (road.pinLocation && road.pinLocation.lat && road.pinLocation.lng) {
          const pinIcon = window.L.divIcon({
            ...
            html: `<div style="background-color: ${getConditionColor(
              road.condition
            )}; width: 18px; height: 18px; border-radius: 50% 50% 50% 0; border: 2px solid white; transform: rotate(-45deg); box-shadow: 2px 2px 5px rgba(0,0,0,0.5);"></div>`,
            iconSize: [18, 18],
            iconAnchor: [9, 18],
            popupAnchor: [0, -18],
          });
          window.L.marker([road.pinLocation.lat, road.pinLocation.lng], {
            icon: pinIcon,
          })
            .addTo(layerGroup)
    ...
    if (filteredRoads.length > 0 && map) {
      const allLatLngs = filteredRoads.flatMap((r) =>
        r.realGps.map((pt) => [pt.lat, pt.lng])
      );
  ...
  useEffect(() => {
    if (
      ...
      !isLeafletLoaded ||
      !surveyorMapContainerRef.current
    )
      return;
  ...
  useEffect(() => {
    if (
      appRole !== 'surveyor' ||
      mobileScreen !== 'pin_map' ||
      !surveyorMapInstanceRef.current
    )
      return;
    ...
    if (pinLocation) {
      if (surveyorMarkerRef.current) {
        ...
        surveyorMarkerRef.current = window.L.marker(
  ...
  const simulateGpsMovement = () => {
    let currentLat = -0.425;
    let currentLng = 117.185;
    setGpsAccuracy('Simulasi');
    setCurrentSpeed(15);
  ...
  const stopRealHardware = () => {
    ...
    if (watchIdRef.current !== null) {
      if (typeof watchIdRef.current === 'number' && gpsAccuracy === 'Simulasi')
  ...
  const saveDraft = (e) => {
    ...
    const compressionRate = Math.round(
      (1 - simplifiedGps.length / realGpsPoints.length) * 100
    );
    ...
    if (compressionRate > 0)
      showToast(
        `Tersimpan! GPS dikompresi ${compressionRate}% (${realGpsPoints.length} ➔ ${simplifiedGps.length} titik)`
      );
  ...
  const syncDataToCloud = async () => {
    ...
    if (!supabase)
      return showToast('Konfigurasi Supabase Anda belum diatur di dalam kode.');
    ...
    try {
      ...
      for (const draft of drafts) {
        ...
        if (draft.videoFile) {
          ...
          const fileName = `video-${Date.now()}-${Math.floor(
            Math.random() * 1000
          )}.${fileExt}`;

          const { error: uploadError } = await supabase.storage
            .from('media')
            .upload(fileName, draft.videoFile);
        ...
        if (dataToUpload.pinLocation)
          dataToUpload.pinLocation = JSON.stringify(dataToUpload.pinLocation);
  ...
  const hapusDataCloud = async (dbId) => {
    if (!supabase) return;
    if (!window.confirm('Hapus permanen rute ini dari database Supabase?'))
      return;
    try {
        ...
        <div className="min-h-screen flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 text-center border-4 border-slate-800">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Pilih peran Anda untuk masuk ke dalam sistem terpadu.
            </p>
            ...
            {!supabase && (
              <div className="mb-6 p-4 bg-amber-50 rounded-xl border border-amber-200 text-amber-800 text-xs text-left">
                ...
                Sistem sedang memuat atau kredensial Anda belum dimasukkan
                dengan benar.
              </div>
            )}
            ...
            <div className="space-y-4">
              <button
                onClick={() => setAppRole('surveyor')}
                className="w-full bg-white border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 text-slate-800 p-4 rounded-2xl flex items-center transition-all group"
              >
              ...
              <button
                ...
                className="w-full bg-white border-2 border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 text-slate-800 p-4 rounded-2xl flex items-center transition-all group"
              >
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col md:max-w-md md:mx-auto md:shadow-2xl md:border-x border-slate-200 relative">
          ...
          <header className="bg-white border-b border-slate-200 py-4 px-5 flex justify-between items-center sticky top-0 z-40">
            ...
            <button
              ...
              className="text-rose-500 font-bold text-xs bg-rose-50 px-3 py-1.5 rounded-lg"
            >
              Keluar
            </button>
          </header>
              ...
              <div className="flex-1 p-6 flex flex-col overflow-y-auto">
                <div className="mb-8 mt-4">
                  ...
                  <p className="text-sm text-slate-500 mt-2">
                    Gunakan penderia peranti untuk merakam rute jalan rusak.
                  </p>
                </div>

                <button
                  onClick={startRealHardware}
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-3xl p-6 mb-4 shadow-xl shadow-blue-600/20 transition-all flex flex-col items-center"
                >
                ...
                <button
                  onClick={() => setMobileScreen('drafts')}
                  className="w-full bg-white border-2 border-slate-200 text-slate-800 rounded-3xl p-5 flex items-center justify-between hover:bg-slate-50 transition-colors"
                >
              ...
              <div className="flex-1 bg-black flex flex-col relative text-white">
                <div className="flex-1 relative bg-slate-900 flex items-center justify-center overflow-hidden">
                  ...
                  <div className="absolute top-6 left-6 bg-black/60 px-4 py-3 rounded-xl text-xs font-bold font-mono backdrop-blur-md border border-white/10">
                    <div className="text-emerald-400 font-bold mb-1 border-b border-white/20 pb-1">
                      SENSOR DATA:
                    </div>
                  ...
                  <div className="absolute top-24 right-6">
                    <button
                      ...
                      className="bg-black/60 hover:bg-black/80 text-white px-4 py-2 rounded-xl text-xs font-bold backdrop-blur-md border border-white/20"
                    >
                      Matikan Kamera
                    </button>
                  </div>
                  ...
                  <div className="absolute bottom-8 w-full px-6 space-y-3">
                    {realGpsPoints.length === 0 && (
                      <button
                        ...
                        className="w-full bg-slate-800 text-white text-xs py-3 rounded-xl font-bold shadow-lg"
                      >
                        Gagal Sinyal GPS? Gunakan Simulasi
                      </button>
                    )}
                    <div className="bg-black/70 backdrop-blur-md p-4 rounded-2xl border border-white/10 text-left text-xs font-mono h-24 overflow-hidden flex flex-col justify-end">
                      ...
                      {realGpsPoints.slice(-2).map((pt, i) => (
                ...
                <div className="p-6 bg-slate-950 flex justify-between items-center z-10 pb-8">
                  ...
                  <button
                    onClick={stopRealHardware}
                    className="bg-white text-black px-8 py-4 rounded-full font-black text-sm shadow-xl flex items-center space-x-2"
                  >
            ...
            {mobileScreen === 'form' && (
              <div className="flex-1 p-6 overflow-y-auto bg-slate-50 text-left">
                ...
                <p className="text-sm text-slate-500 mb-6">
                  Verifikasi informasi rute yang direkam.
                </p>
                ...
                <form onSubmit={saveDraft} className="space-y-5">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Cth: Jl. Poros Utama"
                      className="w-full border border-slate-200 p-4 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border border-slate-200 p-4 rounded-2xl text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500"
                    >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <button
                          ...
                          className={`p-3 rounded-2xl border text-sm font-extrabold transition-all ${
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full border border-slate-200 p-4 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-blue-500"
                      rows="3"
                    ></textarea>
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">
                      Unggah Video ke Supabase (Opsional)
                    </label>
                    <div className="relative border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center bg-white hover:bg-slate-50 transition-colors">
                      ...
                      {uploadedVideoUrl ? (
                        <div className="text-emerald-600 font-bold text-sm flex flex-col items-center">
                          <span className="text-2xl mb-1">✅</span> Video
                          Terlampir (Ketuk tukar)
                        </div>
                      ) : (
                        <div className="text-slate-500 text-sm font-semibold flex flex-col items-center">
                          <span className="text-2xl mb-1">📁</span> Pilih file
                          video dari HP
                        </div>
                      )}
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">
                      Titik Lokasi Kerusakan (Pin)
                    </label>
                    <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center justify-between">
                      <span
                        className={`text-sm font-bold flex items-center ${
                          pinLocation ? 'text-emerald-600' : 'text-slate-500'
                        }`}
                      >
                        {pinLocation ? '📍 Pin Terkunci' : 'Belum ditandai'}
                      </span>
                      <button
                        type="button"
                        onClick={() => setMobileScreen('pin_map')}
                        className="bg-amber-100 text-amber-700 px-4 py-2.5 rounded-xl text-xs font-extrabold"
                      >
                        {pinLocation ? 'Ubah di Peta' : 'Buka Peta'}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 pb-8">
                    <button
                      type="submit"
                      className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold text-base shadow-xl"
                    >
                      Simpan ke Memori Luring (Draft)
                    </button>
                  </div>
                </form>
              </div>
              ...
              <div className="flex-1 flex flex-col bg-slate-100 relative">
                <div className="bg-white px-5 py-4 border-b border-slate-200 flex justify-between items-center z-10 shadow-sm">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Ketuk peta untuk meletakkan pin
                    </p>
                  </div>
                  <button
                    onClick={() => setMobileScreen('form')}
                    className="bg-blue-600 text-white px-5 py-2 rounded-full font-bold text-sm"
                  >
                    Selesai
                  </button>
                </div>
                <div className="flex-1 relative z-0">
                  <div
                    ref={surveyorMapContainerRef}
                    className="absolute inset-0 bg-slate-200"
                  ></div>
                  {!isLeafletLoaded && (
                    <div className="absolute inset-0 flex items-center justify-center bg-slate-100 text-sm font-bold text-slate-400 z-10 pointer-events-none">
                      Memuatkan Peta...
                    </div>
                  )}
                </div>
            ...
            {mobileScreen === 'drafts' && (
              <div className="flex-1 p-6 flex flex-col bg-slate-100 text-left">
                <div className="flex justify-between items-center mb-6 mt-2">
                  <div>
                    <h3 className="text-2xl font-black">Draft Offline</h3>
                  ...
                  <button
                    ...
                    className="bg-slate-200 text-slate-600 p-3 rounded-full hover:bg-slate-300"
                  >
                    Tutup
                  </button>
                </div>
                ...
                <div className="flex-1 space-y-4 overflow-y-auto pb-4">
                  {drafts.length === 0 ? (
                    <div className="text-center text-slate-400 mt-10 text-base font-medium border-2 border-dashed border-slate-300 rounded-3xl p-8">
                      Belum ada survei yang disimpan.
                    </div>
                  ) : (
                    drafts.map((d) => (
                      <div
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundColor: getConditionColor(d.condition),
                          }}
                        ></div>
                        <div className="pl-3">
                ...
                {drafts.length > 0 && (
                  <div className="pt-2 pb-6">
                    <button
                      ...
                      className={`w-full text-white py-4 rounded-2xl font-black text-base flex justify-center items-center space-x-2 shadow-xl ${
                        isDbConnected
                    ...
                    >
                      {isDbConnected ? (
                        ...
                        <span>SERVER SUPABASE TERPUTUS</span>
                      )}
                    </button>
                  </div>
                )}
          ...
          <header className="bg-white border-b border-slate-200 h-16 px-6 flex justify-between items-center flex-shrink-0 z-40 shadow-sm">
            ...
            <div className="flex items-center space-x-4">
              <button
                onClick={() => {
                  fetchRoads();
                  showToast('Memperbarui data dari server...');
                }}
                className="text-blue-600 hover:text-blue-800 font-bold text-sm bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-200"
              >
                Refresh Data
              </button>
              <button
                onClick={() => setAppRole(null)}
                className="text-rose-500 hover:text-white hover:bg-rose-500 border border-rose-200 px-4 py-1.5 rounded-lg text-sm font-bold transition-colors"
              >
                Keluar Dasbor
              </button>
            </div>
          ...
          <main className="flex-1 flex overflow-hidden">
              ...
              <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                ...
                <div className="space-y-3">
                  <select
                    ...
                    className="w-full bg-white border border-slate-200 text-slate-700 text-sm rounded-xl px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500 shadow-sm cursor-pointer"
                  >
                      ...
                      syncedRoads.filter(
                        (r) =>
                          (filterKelurahan === 'Semua' ||
                            r.kelurahan === filterKelurahan) &&
                          (filterKondisi === 'Semua' ||
                            r.condition === filterKondisi)
                      ).length
                    }{' '}
                    Data
                  </span>
                  ...
                  {syncedRoads.length === 0 ? (
                    ...
                    syncedRoads
                      .filter(
                        (road) =>
                          (filterKelurahan === 'Semua' ||
                            road.kelurahan === filterKelurahan) &&
                          (filterKondisi === 'Semua' ||
                            road.condition === filterKondisi)
                      )
                      .map((road) => (
                        <div
                          key={road.dbId || road.id}
                          onClick={() => setSelectedRoad(road)}
                          className={`p-4 rounded-2xl border bg-white cursor-pointer transition-all hover:-translate-y-0.5 ${
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="font-extrabold text-sm text-slate-800 leading-tight pr-2">
                              {road.name}
                            </h4>
                            <span
                              className="text-[10px] font-bold text-white px-2 py-1 rounded-full whitespace-nowrap"
                              style={{
            ...
            <section className="flex-1 flex flex-col relative z-0">
              <div className="flex-1 relative w-full h-full">
                ...
                ></div>
                {!isLeafletLoaded && (
                  <div className="absolute inset-0 flex items-center justify-center bg-slate-100 font-bold text-slate-400 z-10 pointer-events-none">
                    Memuat Peta Leaflet...
                  </div>
                )}
                ...
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm p-3 rounded-xl border border-slate-200 shadow-lg text-xs font-bold text-slate-700 z-[1000] pointer-events-none">
                  ...
                  <div className="flex flex-col space-y-2 mt-2">
                    <div className="flex items-center space-x-2">
                      <span className="w-4 h-1.5 bg-[#10B981] rounded-full"></span>
                      <span>Aspal (Mulus)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="w-4 h-1.5 bg-[#F59E0B] rounded-full"></span>
                      <span>Berbatu (Sedang)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="w-4 h-1.5 bg-[#B45309] rounded-full"></span>
                      <span>Tanah (Rusak)</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="w-4 h-1.5 bg-[#EF4444] rounded-full"></span>
                      <span>Licin (Bahaya)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {selectedRoad && (
                <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 w-11/12 max-w-4xl bg-slate-900 rounded-3xl shadow-2xl border border-slate-700 flex flex-row overflow-hidden z-[1000] animate-fade-in-up">
                  <button
                    ...
                    className="absolute top-4 right-4 text-slate-400 hover:text-white bg-slate-800 p-1.5 rounded-full z-30"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-1">
                        <div className="text-xs font-bold text-blue-400 uppercase tracking-widest">
                          {selectedRoad.kelurahan}
                        </div>
                        <button
                          onClick={() =>
                            hapusDataCloud(selectedRoad.id || selectedRoad.dbId)
                          }
                          className="text-xs text-rose-400 hover:text-rose-300 font-bold px-3 py-1 bg-rose-500/10 rounded-lg border border-rose-500/20"
                        >
                      ...
                      {selectedRoad.pinLocation &&
                        selectedRoad.pinLocation.lat &&
                        selectedRoad.pinLocation.lng && (
                          <div className="mt-3 text-xs text-amber-300 bg-amber-500/10 p-2.5 rounded-lg border border-amber-500/20 inline-flex items-center">
                            <span className="mr-2 text-base">📍</span> Pin
                            Kritis: {selectedRoad.pinLocation.lat.toFixed(5)},{' '}
                            {selectedRoad.pinLocation.lng.toFixed(5)}
                          </div>
                        )}
                    </div>

                    <div className="flex gap-3 text-xs font-bold text-slate-400 mt-4 flex-wrap">
                      <span className="bg-slate-800 px-3 py-1.5 rounded-lg">
                        {selectedRoad.date}
                      </span>
                      <span className="bg-slate-800 px-3 py-1.5 rounded-lg">
                        {selectedRoad.surveyor}
                      </span>
                    </div>
                  </div>
                </div>
